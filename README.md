...
публикацией: иконка, название, автор, версия и changelog. Подложка — половина
...
- О новой версии уведомляют не позже чем через 15 минут после запуска: одной
  периодической работы мало — WorkManager ставит её первый запуск на конец
  первого интервала, то есть через шесть часов. Поэтому рядом идёт разовая
  проверка с задержкой в 15 минут, а об одной и той же версии уведомляют один
  раз (`SettingsStore.notifiedUpdates`).
